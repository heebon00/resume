 ...
 *   확장 상태   86 x 86 원 (20:1770, radius 55)
 *               "Drag" 14px, leading 22.99, 검정 글자
 *               24 x 24 화살표가 좌우로 붙고 간격 25
 *   테두리      1px rgba(255,255,255,0.5) (20:1785)
 * 시안에서는 확장 상태가 모두 opacity 0(호버 상태)으로 저장돼 있다.
 *
 * [지운 것] 시안 20:1765 의 "Explore" 확장(링크·프로젝트 카드에서 뜨던 것)은
 * 요청으로 걷어냈다 — 86 원이 버튼과 글자를 통째로 덮었다. 그 자리에서는
 * 점과 링만 따라간다.
 *
 * `data-cursor="drag"` 가 붙은 요소 위에서만 확장된다.
 * 마우스 기기에서만 동작하고 터치 기기에서는 아예 켜지 않는다.
 */
export default function CustomCursor() {
  const dotRef = useRef(null);
  const ringRef = useRef(null);
  const [mode, setMode] = useState(null); // 'drag' | null
  ...
  useEffect(() => {
    ...
    const onMove = (event) => {
      ...
      const hit = event.target.closest?.('[data-cursor="drag"]');
      setMode(hit ? "drag" : null);
    };
